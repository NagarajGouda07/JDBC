...
import com.xworkz.registration.constants.ConnectionConstants;
...
public class FormRepositoryImpl implements FormRepository {
	...
	public void saveForm(FormDB form) {
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "INSERT INTO form_con_db (name, age, email, contact, password, address) VALUES (?, ?, ?, ?, ?, ?)";
	...
	public FormDB getFormById(int id) {
		...
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "SELECT * FROM form_con_db WHERE id = ?";
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
					preparedStatement.setInt(1, id);
					try (ResultSet resultSet = preparedStatement.executeQuery()) {
						if (resultSet.next()) {
							form = new FormDB(resultSet.getInt("id"), resultSet.getString("name"),
									resultSet.getInt("age"), resultSet.getString("email"), resultSet.getLong("contact"),
	...
	public FormDB getFormByContact(long contact) {
		...
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "SELECT * FROM form_con_db WHERE contact = ?";
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
	...
	public boolean verifyPasswordForUser(String email, String password) {
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "SELECT * FROM form_con_db WHERE  email = ?";
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
	...
	public boolean deleteFormById(int id) {
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "DELETE FROM form_con_db WHERE id = ?";
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
					preparedStatement.setInt(1, id);
	...
	public boolean updateFormByEmail(String email, FormDB updatedForm) {
		try (Connection connection = ConnectionConstants.getConnection()) {
			if (connection != null) {
				String query = "UPDATE form_con_db SET name = ?, age = ?, contact = ?, password = ?, address = ? WHERE email = ?";
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
